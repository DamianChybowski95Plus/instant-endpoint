...
declare namespace Object {
  ...
  type SupportedOptions =
    | TypeParams.SupportedHeaderOptions
    | Object.SupportedOptionsSchema;
  /**
   *
   */
  type SupportedHeaderOptions =
   ...
   *
   */
  type GetSchemas<T extends object> = PickByValue<T, z.Schema> & {
    [key: string]: any;
  };
  /**
   *
  ...
  type ZodInferSchemaValues<TSchemaObject extends SupportedOptions> = {
    [K in keyof Object.GetSchemas<TSchemaObject>]: z.infer<
      Object.GetSchemas<TSchemaObject>[K]
    >;
  };
  /**
   *
  ...
  type ZodInferSchemaValuesWithOptions<TSchemaObject extends SupportedOptions> =
    {
      ...
      [K in keyof PickByValue<TSchemaObject, string>]: PickByValue<
        TSchemaObject,
        string
      >[K];
    };
...
declare namespace Objects {
  ...
  type SchemasObjects = ({
    [K in keyof Pick<
      TypeParams.SupportedEndpointArguments,
      "body" | "searchParams"
    >]: Object.SupportedOptionsSchema;
  } & { [key: string]: any }) &
    ({
   ...
   *
   */
  type OptionsSupportedByEndpointType<
    TEdgeType extends keyof TypeParams.EndpointTypes,
  > = TEdgeType extends keyof Pick<TypeParams.EndpointTypes, "GET">
    ? Partial<
        | Record<
            keyof Pick<TypeParams.SupportedEndpointArguments, "searchParams">,
            Object.SupportedOptionsSchema
          >
        | Record<
            keyof Pick<TypeParams.SupportedEndpointArguments, "headers">,
            Object.SupportedHeaderOptions
          >
      >
    : TEdgeType extends keyof Pick<TypeParams.EndpointTypes, "POST">
      ? Partial<
          ...
          > &
            ...
            > &
            Record<
              keyof Pick<TypeParams.SupportedEndpointArguments, "body">,
              Object.SupportedOptionsSchema
            >
        >
   ...
   */
  type GetSchemas<T extends Objects.SchemasObjects> = {
    [K in keyof T]: Object.GetSchemas<T[K]>;
  };
  /**
   *
   */
  type ZodInferSchemaValues<T extends Objects.SchemasObjects> = {
    [K in keyof T]: Object.ZodInferSchemaValues<T[K]>;
  };
  /**
   *
   */
...
>(
  ...
  function getRequiredHeaders<THeaders extends Object.SupportedOptionsSchema>(
    ...
    const requiredSearchParamsSchemaEntries: [keyof THeaders, z.Schema][] =
      Object.entries(requiredHeadersSchema);
    ...
    const requiredHeaders = Object.fromEntries(
      requiredSearchParamsSchemaEntries.map((entry) => {
        ...
        const requiredHeaderAny = requiredHeaderUnknown
          ? safeParseJson(requiredHeaderUnknown)
          : undefined;
  ...
  function getRequiredUrlParams<TParams extends Object.SupportedOptionsSchema>(
    ...
    const requiredSearchParamsSchemaEntries: [keyof TParams, z.Schema][] =
      Object.entries(urlParamsSchema);
    ...
    const requestUrlSearchParams = Object.fromEntries(
      requiredSearchParamsSchemaEntries.map((entry) => {
        ...
        const paramString = paramUnknown
          ? safeParseJson(paramUnknown)
          : undefined;
        const requiredParam =
  ...
  function getRequiredBody<TBody extends Object.SupportedOptionsSchema>(
    ...
    const requiredBodySchemaEntries: [keyof TBody, z.Schema][] =
      Object.entries(requiredBodySchema);
    ...
    const requiredBody = Object.fromEntries(
      requiredBodySchemaEntries.map((entry) => {
        if (
          requestsBodyKeys.includes(entry[0] as string) &&
          entry[1].safeParse(requestsBody[entry[0]]).success
        ) {
  ...
  return [
    ...
    async (request: NextRequest) => {
        ...
        ...("body" in requiredForTransaction && requiredForTransaction.body
          ? {
              body: getRequiredBody(
                await request.json(),
                requiredForTransaction.body,
              ),
            }
          : undefined),
        ...("headers" in requiredForTransaction &&
        requiredForTransaction.headers
          ? {
              headers: getRequiredHeaders(
                request,
                extractSchemaObject(requiredForTransaction.headers),
              ),
            }
          : undefined),
        ...("searchParams" in requiredForTransaction &&
        requiredForTransaction.searchParams
          ? {
              searchParams: getRequiredUrlParams(
                request,
                requiredForTransaction.searchParams,
              ),
            }
      ...
      if (Object.values(requiredAfterValidation).includes(null)) {
        console.log(
          "InstantEndpoint - transaction requirements weren't met",
          requiredAfterValidation,
        );
      ...
      const callbackReturn = await callback(
        request,
        requiredAfterValidation as Objects.ZodInferSchemaValuesWithOptions<TRequired>,
      );
      return callbackReturn instanceof NextResponse
        ? callbackReturn
        : NextResponse.json(callbackReturn);
    },
    ...
    async (requiredToCall, pushRouterFunction) => {
      ...
      const transactionParams = {
        ...("body" in requiredToCall && requiredToCall.body
          ? { body: JSON.stringify(requiredToCall.body) }
          : null),
        ...("headers" in requiredToCall && requiredToCall.headers
          ? { headers: strinfigyValues(requiredToCall.headers) }
          : null),
        ...("searchParams" in requiredToCall && requiredToCall.searchParams
          ? {
              searchParams: optionsToSearchParams(
                requiredToCall.searchParams as { [key: string]: string },
              ),
            }
          : null),
      };
      ...
      if (edgeType === "GETREDIRECT" && pushRouterFunction) {
        ...
        const searchParamsString = transactionParams.searchParams
          ? transactionParams.searchParams
          : "";
        const res = await fetch(apiUrl + searchParamsString, {
          method: "GET",
          ...(transactionParams.headers
            ? { headers: transactionParams.headers }
            : {}),
        });
        return (await res.json()) as unknown as Awaited<
        ...
        return res.json() as unknown as Awaited<ReturnType<TCallback>> & {
          [key: string]: any;
        };
      }
    },
